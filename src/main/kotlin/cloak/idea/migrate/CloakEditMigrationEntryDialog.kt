...
//        buttonGroup.add(myRbPackage)
        ...
        myOldNameField.document.addDocumentListener(documentAdapter)
        myNewNameField.document.addDocumentListener(documentAdapter)
        return panel
    }
    ...
    fun setEntry(entry: CloakMigrationMapEntry) {
        myOldNameField.text = entry.oldName
        myNewNameField.text = entry.newName
//        myRbPackage!!.isSelected = entry.type == Migr.PACKAGE
        myRbClass!!.isSelected = entry.type == MigrationEntryType.Class
        validateOKButton()
    }

    fun updateEntry(entry: CloakMigrationMapEntry) : CloakMigrationMapEntry = entry.copy(
        oldName = myOldNameField.text.trim { it <= ' ' },
        newName = myNewNameField.text.trim { it <= ' ' },
        type =/*when(foo){bar}*/ MigrationEntryType.Class
    )
//        entry.oldName =
//        entry.newName =
